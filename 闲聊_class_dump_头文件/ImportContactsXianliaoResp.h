//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSMutableArray, NSString;

@interface ImportContactsXianliaoResp : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) long long contactDataVersion; // @dynamic contactDataVersion;
@property(nonatomic) int errorCode; // @dynamic errorCode;
@property(copy, nonatomic) NSString *errorMessage; // @dynamic errorMessage;
@property(nonatomic) int newNotCertifiedNum; // @dynamic newNotCertifiedNum;
@property(retain, nonatomic) NSMutableArray *userArray; // @dynamic userArray;
@property(readonly, nonatomic) unsigned long long userArray_Count; // @dynamic userArray_Count;

@end

